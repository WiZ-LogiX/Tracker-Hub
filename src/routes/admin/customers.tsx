...
interface Customer {
  ...
  governorate: string | null; address: string | null; created_at: string;
}
...
function CustomersPage() {
  ...
  async function load() {
    ...
    setCustomers(loaded); setFilteredCustomers(loaded);
  }
  ...
  function openNew() { setEditing(null); setForm(blankCustomer); setOpen(true); }
  ...
  async function remove(id: string) {
    ...
    if (error) return toast.error(error.message);
    toast.success(t("customers.deleted")); load();
  }

  function formatDate(d: string) {
    const locale = i18n.language === 'en' ? 'en-US' : i18n.language === 'fr' ? 'fr-FR' : 'ar-EG';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h1 className="font-serif text-3xl font-bold">{t("customers.title")}</h1>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-lg">{t("customers.list")}</CardTitle>
          <div className="relative max-w-xs">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
            <Input placeholder={t("customers.searchPlaceholder")} value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-9" />
          </div>
        </CardHeader>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{t("customers.governorate")}</TableHead>
                <TableHead>{t("customers.address")}</TableHead>
                <TableHead>{t("customers.addedDate")}</TableHead>
            ...
            <TableBody>
              ...
              {filteredCustomers.map(c => (
                <TableRow key={c.id}>
                  ...
                  <TableCell><div className="flex items-center gap-1"><Phone className="h-3.5 w-3.5 text-muted-foreground" /><span>{c.phone}</span></div></TableCell>
                  <TableCell>{c.email ? <div className="flex items-center gap-1"><Mail className="h-3.5 w-3.5 text-muted-foreground" /><span className="text-sm">{c.email}</span></div> : <span className="text-muted-foreground">—</span>}</TableCell>
                  <TableCell>{c.governorate ? <div className="flex items-center gap-1"><MapPin className="h-3.5 w-3.5 text-muted-foreground" /><span>{c.governorate}</span></div> : <span className="text-muted-foreground">—</span>}</TableCell>
                  <TableCell>{c.address ? <div className="flex items-center gap-1 max-w-xs truncate"><Building2 className="h-3.5 w-3.5 text-muted-foreground" /><span className="truncate">{c.address}</span></div> : <span className="text-muted-foreground">—</span>}</TableCell>
