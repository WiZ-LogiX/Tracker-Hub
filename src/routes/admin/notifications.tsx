...
function NotificationsPage() {
  ...
  async function onSendTest(e: React.FormEvent) {
    ...
    try {
      ...
      if (r.status === "sent") toast.success("تم الإرسال إلى n8n");
      else if (r.status === "skipped") toast.warning(`تخطّي: ${r.reason}`);
      else toast.error(`فشل: ${r.error ?? r.http ?? "unknown"}`);
      load();
    } catch (err: any) {
  ...
  const color = (s: string) => s === "sent" ? "default" : s === "failed" ? "destructive" : "secondary";
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Outgoing customer notifications via n8n webhook.</p>
        </div>
        <Button variant="outline" size="sm" onClick={load} disabled={loading}><RefreshCw className="h-4 w-4 mr-2" />Refresh</Button>
      </div>

      <Card>
        <CardHeader><CardTitle className="text-lg">WhatsApp test</CardTitle></CardHeader>
        <CardContent>
          <form onSubmit={onSendTest} className="grid sm:grid-cols-[200px_1fr_auto] gap-3 items-end">
            ...
            <Button type="submit" disabled={sending} className="gap-2"><Send className="h-4 w-4" />{sending ? "..." : "Send test"}</Button>
          </form>
          <p className="text-xs text-muted-foreground mt-2">يرسل payload إلى <code>N8N_NOTIFY_WEBHOOK_URL</code> ويُسجّل النتيجة في الجدول أدناه.</p>
        </CardContent>
      </Card>

      <Card>
        <CardHeader><CardTitle className="text-lg">Recent (last 200)</CardTitle></CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {rows.map(r => (
                <TableRow key={r.id}>
                  ...
                  <TableCell><Badge variant={color(r.status) as any}>{r.status}</Badge></TableCell>
                  <TableCell className="text-xs text-destructive max-w-[260px] truncate">{r.error}</TableCell>
                </TableRow>
              ))}
              {rows.length === 0 && (
                <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-8">No notifications yet.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
