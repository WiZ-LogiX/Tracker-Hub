...
function QuoteBuilder() {
  ...
  async function applyDiscount() {
    ...
    if (!data) { toast.error(t("quoteBuilder.discountInvalid")); setAppliedDiscount(null); return; }
    setAppliedDiscount(data);
    toast.success(t("quoteBuilder.discountApplied"));
  }

  async function saveQuote(status: 'draft' | 'sent') {
    if (!customerId) return toast.error(t("quoteBuilder.selectCustomerFirst"));
    if (calculatedItems.some(ci => !ci.breakdown)) return toast.error(t("quoteBuilder.incompleteItems"));
    setSaving(true);
    try {
      ...
      const { quoteId } = await createQuoteFn({
        data: {
          ...
          total: totals.total,
          notes: notes || null,
          snapshot: { items: calculatedItems.map(c => ({ product: c.product?.name_ar, material: c.material?.name_ar, finish: c.finish?.name_ar, breakdown: c.breakdown })) },
          items: itemsToInsert,
        },
      });
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-serif text-3xl font-bold">{t("quoteBuilder.title")}</h1>
        <p className="text-sm text-muted-foreground mt-1">{t("quoteBuilder.subtitle")}</p>
      </div>

      <Card>
        <CardHeader><CardTitle className="text-lg">{t("quotes.customer")}</CardTitle></CardHeader>
        <CardContent>
          <Label>{t("quoteBuilder.customerSelect")}</Label>
          ...
          {calculatedItems.map((ci, idx) => (
            <div key={idx} className="border rounded-lg p-4 space-y-3 bg-muted/30">
              ...
              <div className="grid md:grid-cols-3 gap-3">
                <div>
                  <Label>{t("quoteBuilder.product")}</Label>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex gap-2">
            <Input placeholder={t("quoteBuilder.discountCode")} value={discountCode} onChange={e => setDiscountCode(e.target.value)} />
            <Button variant="outline" onClick={applyDiscount}>{t("quoteBuilder.applyDiscount")}</Button>
          </div>
          ...
          <div className="space-y-1 text-sm">
            ...
            {totals.discountAmount > 0 && <div className="flex justify-between text-secondary"><span>{t("quotes.discount")}</span><span>− {formatEGP(totals.discountAmount)}</span></div>}
          ...
          <div className="flex gap-2 pt-3">
            <Button variant="outline" disabled={saving} onClick={() => saveQuote('draft')}>{t("quoteBuilder.saveAsDraft")}</Button>
            <Button disabled={saving} onClick={() => saveQuote('sent')}>{t("quoteBuilder.sendQuote")}</Button>
          </div>
        </CardContent>
      </Card>
    </div>
