...
function ConfiguratorBuilder() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from('material_variants').select('*').eq('active', true),
    ...
    ]).then(([t, c, m, mv, sp, f, v, a, cu, pf, pr, wr]) => {
      setTemplates(t.data ?? []); setCategories(c.data ?? []); setMaterials(m.data ?? []);
      setVariants(mv.data ?? []); setSuppliers(sp.data ?? []);
  ...
  const computed = useMemo(() => items.map(it => {
    ...
    const material = materials.find(m => m.id === (it.material_id ?? variant?.material_id));
    const supplier = variant?.supplier_id ? supplierById[variant.supplier_id] : null;
    ...
    const matRate = Number(variant?.price_per_unit ?? 0);
    ...
    const autoWastage = lookupWastage(material?.type, it.dimension_value);
  ...
  }), [items, templates, materials, variants, suppliers, finishes, veneers, accessories, globalFactors, activeRule, wastageRules]);
  ...
  async function saveQuote(status: 'draft' | 'sent') {
    ...
    const { data: quote, error } = await supabase.from('quotes').insert({
      ...
      snapshot: { rule_version: activeRule?.version, items: computed.map(c => ({
        ...
        supplier_country: c.variant?.country_of_origin ?? null,
    ...
    if (insertedItems) {
      const configs = insertedItems.map((qi, i) => ({
        ...
        selections: {
          variant_id: computed[i].variant?.id, supplier_country: computed[i].variant?.country_of_origin,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-serif text-3xl font-bold flex items-center gap-2"><Sparkles className="h-6 w-6 text-primary" /> مكوّن المنتج (Configurator)</h1>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {computed.map((ci, idx) => {
            const it = ci.it;
            const matVariants = variants.filter(v => !it.material_id || v.material_id === it.material_id);
            return (
              <div key={idx} className="border rounded-lg p-4 space-y-3 bg-muted/30">
                ...
                <div className="grid md:grid-cols-3 gap-3">
                  ...
                  <div>
                    <Label>المورد / المنشأ</Label>
                    <Select value={it.variant_id ?? ''} onValueChange={v => updateItem(idx, { variant_id: v })}>
                      <SelectTrigger><SelectValue placeholder="اختر" /></SelectTrigger>
                      <SelectContent>
                        {matVariants.map(v => {
                          const sup = v.supplier_id ? supplierById[v.supplier_id] : null;
                          return (
                            <SelectItem key={v.id} value={v.id}>
                              {(sup?.name ?? '—')} • {v.country_of_origin ?? '—'} • {formatEGP(Number(v.price_per_unit))}
                            </SelectItem>
                          );
                        })}
                      </SelectContent>
                    ...
                    {ci.variant && (
                      <div className="mt-1 text-[11px] text-muted-foreground">
                        {ci.supplier?.name ?? '—'} • {ci.variant.country_of_origin ?? '—'}
                      </div>
                    )}
                  </div>
                  <div>
