...
function OrdersPage() {
  ...
  async function openOrder(o: any) {
    ...
    setLogs(l ?? []); setPhotos(ph ?? []); setAssignments(as ?? []); setInspections(qc ?? []);
  }
  ...
  async function uploadPhotos(files: File[], caption: string) {
    ...
    try {
      ...
      const { uploads } = await getBatchUploadUrls({ data: { files: fileInfos, entityType: 'production-photos', entityId: selected.id } });
      let ok = 0, fail = 0;
      ...
      if (fail) toast.error(t("orders.upload", { n: fail }) + " (فشل)");
  ...
  async function recordQC(stage: OrderStage, passed: boolean, qcNotes: string) {
    if (!selected) return;
    const { error } = await supabase.from('qc_inspections').insert({ order_id: selected.id, stage: stage as any, passed, notes: qcNotes || null, inspector_id: user?.id });
    if (error) return toast.error(error.message);
    if (!passed) {
      await supabase.from('remakes').insert({ order_id: selected.id, reason: qcNotes || `QC failed: ${stage}`, status: 'open', created_by: user?.id });
  ...
  async function advance(o: any) {
    ...
    const { error } = await supabase.from('orders').update({ current_stage: next as any, ...(next === 'delivered' ? { delivered_at: new Date().toISOString() } : {}) }).eq('id', o.id);
    if (error) return toast.error(error.message);
    await supabase.from('production_logs').insert({ order_id: o.id, stage_from: o.current_stage as any, stage_to: next as any, transitioned_by: user?.id, notes: note || null });
    try {
    ...
    await load();
    if (selected?.id === o.id) { const updated = (await supabase.from('orders').select('*, customers(name,phone)').eq('id', o.id).single()).data; openOrder(updated); }
  }
  async function sendTrackingLink(o: any) {
    try {
      ...
      else toast.error(`Failed${r?.http ? ` (HTTP ${r.http})` : ''}`);
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="font-serif text-3xl font-bold">{t("orders.title")}</h1><p className="text-sm text-muted-foreground mt-1">{t("orders.subtitle")}</p></div>
      <div className="grid lg:grid-cols-3 gap-4">
        {ORDER_STAGES.slice(0, -1).map(stage => {
          ...
          return (
            <Card key={stage}>
              <CardHeader className="pb-2"><CardTitle className="text-sm flex items-center justify-between"><span>{STAGE_LABEL_AR[stage]}</span><Badge variant="outline">{ordersInStage.length}</Badge></CardTitle></CardHeader>
                  ...
                  <Dialog key={o.id}>
                    ...
                    <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                      ...
                      {selected && <OrderDetail
                        ...
                        onRecordQC={recordQC} t={t} i18n={_i18n}
                      />}
                    </DialogContent>
                  </Dialog>
                ))}
      ...
      {orders.filter(o => o.current_stage === 'completed').length > 0 && (
        <Card><CardHeader><CardTitle className="text-lg">{t("orders.completed")}</CardTitle></CardHeader><CardContent className="space-y-2">
      ...
      {orders.length === 0 && <Card><CardContent className="p-12 text-center text-muted-foreground">{t("orders.noOrders")}</CardContent></Card>}
    </div>
  );
}

function OrderDetail({ o, logs, photos, assignments, inspections, workers, note, setNote, onAdvance, onSendLink, onUpload, onDeletePhoto, onAssign, onUpdateAssignment, onDeleteAssignment, onRecordQC, t, i18n }: any) {
  ...
  return (
    <div className="space-y-4 text-sm">
      <div className="grid grid-cols-2 gap-3">
        <div><div className="text-xs text-muted-foreground">{t("orders.customer")}</div><div>{o.customers?.name}</div></div>
        <div><div className="text-xs text-muted-foreground">{t("orders.phone")}</div><div>{o.customers?.phone}</div></div>
        <div><div className="text-xs text-muted-foreground">{t("orders.total")}</div><div>{formatEGP(Number(o.total))}</div></div>
        <div><div className="text-xs text-muted-foreground">{t("orders.deposit")}</div><div>{formatEGP(Number(o.deposit))}</div></div>
      </div>
      <div>
        <div className="text-xs text-muted-foreground mb-2">{t("orders.stageProgress")}</div>
      ...
      <div className="pt-2 border-t space-y-2">
        <div className="text-xs text-muted-foreground">{t("orders.assignments", { n: assignments.length })}</div>
        {assignments.length === 0 && <div className="text-xs text-muted-foreground">{t("orders.noAssignments")}</div>}
        {assignments.map((a: any) => (
          <div key={a.id} className="border rounded-md p-2 text-xs space-y-1">
            ...
            <div className="flex gap-1">
              ...
              <Button size="sm" variant="ghost" className="h-6 text-[10px] text-destructive" onClick={() => onDeleteAssignment(a)}>{t("orders.delete")}</Button>
            </div>
          </div>
        ))}
        <AssignWorkerForm currentStage={o.current_stage} workers={workers} onAssign={onAssign} t={t} />
      </div>
      <div className="pt-2 border-t space-y-2">
        <div className="text-xs text-muted-foreground">{t("orders.qcInspections", { n: inspections.length })}</div>
        {inspections.slice(0, 5).map((q: any) => (
          <div key={q.id} className="text-xs border rounded-md p-2 flex justify-between items-start">
            <div>
              ...
              <div className="text-[10px] text-muted-foreground mt-1">{new Date(q.created_at).toLocaleString()}</div>
            </div>
            <Badge variant={q.passed ? 'default' : 'destructive'}>{q.passed ? t("orders.qcPass") : t("orders.qcFail")}</Badge>
          </div>
        ))}
        <QCForm currentStage={o.current_stage} onSubmit={onRecordQC} t={t} />
      </div>
      <div className="pt-2 border-t space-y-3">
        <div className="text-xs text-muted-foreground">{t("orders.productionPhotos")} ({photos?.length || 0})</div>
        {photos?.length > 0 && (() => {
          const groups: Record<string, any[]> = {};
          for (const p of photos) { const k = p.stage || 'unknown'; (groups[k] ||= []).push(p); }
          const order = ORDER_STAGES.filter(s => groups[s]);
          return <div className="space-y-3 max-h-72 overflow-auto pr-1">
            {order.map(stage => <div key={stage} className="space-y-1">
              <div className="text-xs font-medium text-gold">{STAGE_LABEL_AR[stage]} ({groups[stage].length})</div>
              <div className="grid grid-cols-3 gap-2">{groups[stage].map((p: any) => <div key={p.id} className="relative group"><img src={p.photo_url} className="w-full aspect-square object-cover rounded border" /><button type="button" onClick={() => onDeletePhoto(p)} className="absolute top-1 left-1 bg-black/60 text-white text-[10px] rounded px-1.5 py-0.5 opacity-0 group-hover:opacity-100">{t("orders.delete")}</button></div>)}</div>
            </div>)}
          </div>;
        })()}
        <PhotoUploader onUpload={onUpload} t={t} />
      </div>
      <div className="pt-2 border-t"><InternalNotes entityType="order" entityId={o.id} /></div>
      <div className="pt-2 border-t space-y-2">
        <div className="text-xs text-muted-foreground">{t("orders.sendTrackingLink")}</div>
        <Button type="button" variant="outline" className="w-full" onClick={onSendLink}>{t("orders.sendTrackingLink")}</Button>
      </div>
      {next && <div className="space-y-2 pt-2 border-t">
        ...
        <Button className="w-full" onClick={onAdvance} disabled={!gateOk}>{t("orders.advance", { stage: STAGE_LABEL_AR[next] })}</Button>
      </div>}
    </div>
  );
}

function AssignWorkerForm({ currentStage, workers, onAssign, t }: any) {
  ...
  return (
    <div className="flex flex-wrap gap-2 p-2 border rounded-md bg-muted/30">
      ...
      <Select value={workerId} onValueChange={setWorkerId}>
        <SelectTrigger className="h-8 text-xs flex-1 min-w-32"><SelectValue placeholder={t("orders.worker")} /></SelectTrigger>
        <SelectContent>{workers.map((w: any) => <SelectItem key={w.id} value={w.id}>{w.name}{w.role ? ` (${w.role})` : ''}</SelectItem>)}</SelectContent>
      </Select>
      <Button size="sm" className="h-8 text-xs" disabled={!workerId} onClick={() => { onAssign(stage, workerId); setWorkerId(""); }}>{t("orders.assignWorker")}</Button>
    </div>
  );
}

function QCForm({ currentStage, onSubmit, t }: any) {
  const [stage, setStage] = useState<OrderStage>(currentStage);
  ...
  return (
    <div className="p-2 border rounded-md bg-muted/30 space-y-2">
      ...
      <Textarea rows={2} placeholder={t("orders.qcNotes")} value={notes} onChange={e => setNotes(e.target.value)} className="text-xs" />
      <div className="flex gap-2">
        <Button size="sm" className="flex-1 h-8 text-xs" onClick={() => { onSubmit(stage, true, notes); setNotes(""); }}>{t("orders.qcPass")}</Button>
        <Button size="sm" variant="destructive" className="flex-1 h-8 text-xs" onClick={() => { onSubmit(stage, false, notes); setNotes(""); }}>{t("orders.qcFail")}</Button>
      </div>
    </div>
  );
}

function PhotoUploader({ onUpload, t }: any) {
  ...
  return (
    <div className="flex flex-col gap-2 p-2 border rounded-md bg-muted/30">
      ...
      {files.length > 0 && <div className="text-xs text-muted-foreground">{files.length} file(s) selected</div>}
      <input type="text" placeholder={t("orders.photoCaption")} value={caption} onChange={e => setCaption(e.target.value)} className="text-xs border rounded px-2 py-1 bg-background" />
      <Button size="sm" disabled={files.length === 0 || busy} onClick={async () => {
        if (files.length === 0) return; setBusy(true); await onUpload(files, caption); setFiles([]); setCaption(""); setBusy(false);
      }}>{busy ? t("orders.uploading", { n: files.length }) : t("orders.upload", { n: files.length || '' })}</Button>
    </div>
  );
}