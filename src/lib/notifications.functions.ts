  ...
  .handler(async ({ data }) => {
    ...
    if (!webhook) {
      await supabaseAdmin.from("notification_log").insert({
        entity_type: "test", entity_id: null, reference: "TEST",
        event: "test", channel: "whatsapp", recipient: data.phone, status: "skipped",
        payload: payload as any, error: "N8N_NOTIFY_WEBHOOK_URL not configured",
      });
      return { status: "skipped", reason: "no_webhook" };
    }
    try {
      const headers: Record<string, string> = { "Content-Type": "application/json" };
      if (process.env.N8N_WEBHOOK_TOKEN) headers["X-Lovable-Token"] = process.env.N8N_WEBHOOK_TOKEN;
      const res = await fetch(webhook, { method: "POST", headers, body: JSON.stringify(payload) });
      const respText = await res.text();
      const status = res.ok ? "sent" : "failed";
      await supabaseAdmin.from("notification_log").insert({
        entity_type: "test", entity_id: null, reference: "TEST",
        event: "test", channel: "whatsapp", recipient: data.phone, status,
      ...
      await supabaseAdmin.from("notification_log").insert({
        entity_type: "test", entity_id: null, reference: "TEST",
        event: "test", channel: "whatsapp", recipient: data.phone, status: "failed",
        payload: payload as any, error: String(err?.message ?? err),
      });
...
async function loadEntity(entityType: string, entityId: string) {
  if (entityType === "quote") {
    const { data } = await supabaseAdmin
      .from("quotes")
      .select("id, quote_number, total, customer_id, customers(name, phone, email)")
  ...
  if (entityType === "order") {
    const { data } = await supabaseAdmin
      .from("orders")
      .select("id, order_number, current_stage, expected_delivery, customer_id, customers(name, phone, email)")
      .eq("id", entityId).maybeSingle();
    return data ? {
  ...
  if (entityType === "invoice") {
    const { data } = await supabaseAdmin
      .from("invoices")
      .select("id, invoice_number, total, customer_id, customers(name, phone, email)")
      .eq("id", entityId).maybeSingle();
    return data ? {
...
export const sendNotification = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    let { data: tpl } = await supabaseAdmin
      ...
      .select("subject, body")
      .eq("event", data.event).eq("channel", channel).eq("language", language).eq("active", true)
      .maybeSingle();
    if (!tpl) {
      const { data: fallback } = await supabaseAdmin
        .from("notification_templates")
        .select("subject, body")
        .eq("event", data.event).eq("channel", channel).eq("language", "en").eq("active", true)
        .maybeSingle();
      tpl = fallback;
    }
    ...
    const link = siteUrl ? `${siteUrl}/track?ref=${encodeURIComponent(entity.reference)}` : `/track?ref=${encodeURIComponent(entity.reference)}`;
    ...
    if (!webhook) {
      await supabaseAdmin.from("notification_log").insert({
        entity_type: data.entityType, entity_id: data.entityId, reference: entity.reference,
        event: data.event, channel, recipient, status: "skipped",
        payload: payload as any, error: "N8N_NOTIFY_WEBHOOK_URL not configured",
      });
      return { status: "skipped", reason: "no_webhook" };
    }
    ...
    try {
      ...
      await supabaseAdmin.from("notification_log").insert({
        entity_type: data.entityType, entity_id: data.entityId, reference: entity.reference,
        event: data.event, channel, recipient, status,
        payload: payload as any,
        response: { status: res.status, body: respText.slice(0, 2000) } as any,
        error: res.ok ? null : `HTTP ${res.status}`,
      });
      ...
      await supabaseAdmin.from("notification_log").insert({
        entity_type: data.entityType, entity_id: data.entityId, reference: entity.reference,
        event: data.event, channel, recipient, status: "failed",
        payload: payload as any, error: String(err?.message ?? err),
      });
      return { status: "failed", error: String(err?.message ?? err) };
    }
