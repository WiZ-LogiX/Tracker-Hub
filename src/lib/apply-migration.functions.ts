...
export const applyWastageRulesMigration = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    const sql = (query: string) => (supabaseAdmin.rpc as any)('exec', { query });

    try {
      // 1. Add material_id column if not exists
      const { error: colError } = await sql(`
      ...
      const { error: idxError } = await sql(`
      ...
      const { error: migrateError } = await sql(`
