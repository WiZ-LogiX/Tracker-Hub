...
export const applyWastageRulesMigration = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ context }) => {
    ...
    try {
      ...
      const { error: colError } = await (supabaseAdmin.rpc as any)('exec', {
        query: \`
      ...
      const { error: fkError } = await (supabaseAdmin.rpc as any)('exec', {
        query: \`
          DO \$\$
          BEGIN
            IF NOT EXISTS (
      ...
      const { error: idxError } = await (supabaseAdmin.rpc as any)('exec', {
        query: \`
      ...
      const { error: migrateError } = await (supabaseAdmin.rpc as any)('exec', {
        query: \`
          INSERT INTO public.wastage_rules (material_id, wastage_pct, active, created_at)
            ...
            WHERE m.wastage_pct IS NOT NULL              AND m.wastage_pct > 0
              AND wr.material_id IS NULL          ON CONFLICT (material_id) DO NOTHING;
        \`
      });
